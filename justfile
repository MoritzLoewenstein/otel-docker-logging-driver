...
PKG_DIR := justfile_directory() + "/plugin/package"
...
plugin-remove:
    - docker plugin disable -f {{PLUGIN_NAME}}:{{PLUGIN_TAG}} >/dev/null 2>&1
    docker plugin rm -f {{PLUGIN_NAME}}:{{PLUGIN_TAG}}

plugin-up: plugin-build
    - docker plugin disable -f {{PLUGIN_NAME}}:{{PLUGIN_TAG}} >/dev/null 2>&1
    - docker plugin rm -f {{PLUGIN_NAME}}:{{PLUGIN_TAG}} >/dev/null 2>&1
    docker plugin create {{PLUGIN_NAME}}:{{PLUGIN_TAG}} {{PKG_DIR}}
    docker plugin enable {{PLUGIN_NAME}}:{{PLUGIN_TAG}}
